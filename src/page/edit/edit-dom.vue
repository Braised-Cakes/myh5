<template>
    <div v-if="hasSelectedOneItem && curItem.style">
        ...
        <el-scrollbar v-if="navIndex == 0"
            ...
            <div style="padding:12px 20px;">
                ...
                <div class="style-item"
                    ...
                    <label>行高</label>
                    <el-input-number lazy
                        :step="0.1"
                        @change="updateItem({key:'style', val:{'line-height':$event}})"
                        :value="curItem.style['line-height'] || 1.5"
                        :min="0"
                        :max="3"></el-input-number>
                </div>
                <div class="style-item"
                    v-if="rules('letter-spacing')">
                    <label>字距</label>
                    <el-input-number :step="1"
                        @change="updateItem({key:'style', val:{'letter-spacing':($event / 100).toFixed(2) + 'em'}})"
                        :value="Math.round(100 * parseFloat(curItem.style['letter-spacing'] || 0))"
                        :min="0"
                        :max="100"></el-input-number>
                </div>
                <div class="style-item"
                    v-if="rules('font-size')">
                    <label>字号</label>
                    <el-input-number :step="2"
                        @change="updateItem({key:'style', val:{'font-size':$event + 'px'}})"
                        :value="parseInt(curItem.style['font-size']) || 12"
                        :min="12"
                        :max="96"></el-input-number>
                </div>
                <div class="style-item"
                    v-if="rules('color')">
                    <label>文字颜色</label>
                    ...
                    <ul class="color-list">
                        <li @click="updateItem({key:'style', val:{'color':item}})"
                    ...
                    <div style="padding:12px 20px;">
                        <div class="style-item">
                            ...
                            <el-color-picker @active-change="updateItem({key:'style', val:{'background-color':$event}})"
                                ...
                                show-alpha></el-color-picker>
                            <ul class="color-list">
                                <li @click="updateItem({key:'style', val:{'background-color':item}})"
                                    :style="{'background':item == 'rgba(0,0,0,0)' ? 'url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAwAAAAMCAIAAADZF8uwAAAAGUlEQVQYV2M4gwH+YwCGIasIUwhT25BVBADtzYNYrHvv4gAAAABJRU5ErkJggg==)' : item}"
                                    :key="item"
                                    v-for="item in colorList"></li>
                            </ul>
                        ...
                        <div class="style-item">
                            <label>透明度</label>
                            <el-input-number :step="1"
                                @change="updateItem({key:'style', val:{'opacity':($event / 100).toFixed(2)}})"
                                :value="Math.round((curItem.style['opacity'] || 1) * 100)"
                                :min="0"
                                :max="100"></el-input-number>
                        </div>
                        <div class="style-item">
                            <label>宽度</label>
                            <el-input-number :step="1"
                                @change="updateItem({key:'style', val:{'width': $event + 'px'}})"
                                :value="parseInt(curItem.style['width'])"
                                :min="0"></el-input-number>
                        </div>
                        <div class="style-item">
                            <label>高度</label>
                            <el-input-number :step="1"
                                @change="updateItem({key:'style', val:{'height': $event + 'px'}})"
                                :value="parseInt(curItem.style['height'])"
                                :min="0"></el-input-number>
                        </div>
                        <div class="style-item">
                            <label>x</label>
                            <el-input-number :step="1"
                                @change="updateItem({key:'style', val:{'left': $event + 'px'}})"
                                :value="parseInt(curItem.style['left'])"
                                :min="0"></el-input-number>
                        </div>
                        <div class="style-item">
                            <label>y</label>
                            <el-input-number :step="1"
                                @change="updateItem({key:'style', val:{'top': $event + 'px'}})"
                                :value="parseInt(curItem.style['top'])"
                                :min="0"></el-input-number>
                        </div>
                        <div ng-if="ability.advanceStyle.borderColor"
                            class="tab-setting-line">
                            <div class="tab-setting-line-title">边框</div>
                        </div>
                        <div class="style-item">
                            <label>边框样式</label>
                            <el-select @change="updateItem({key:'style', val:{'border-style': $event}})"
                                :value="curItem.style['border-style'] || 'solid'"
                                placeholder="请选择">
                                <el-option v-for="item in options222"
                                    :key="item.value"
                                    :label="item.label"
                                    :value="item.value">
                                </el-option>
                        ...
                        <div class="style-item">
                            ...
                            <el-color-picker @active-change="updateItem({key:'style', val:{'border-color':$event}})"
                                ...
                                show-alpha></el-color-picker>
                            <ul class="color-list">
                                <li @click="updateItem({key:'style', val:{'border-color':item}})"
                                    :style="{'background':item == 'rgba(0,0,0,0)' ? 'url(data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAwAAAAMCAIAAADZF8uwAAAAGUlEQVQYV2M4gwH+YwCGIasIUwhT25BVBADtzYNYrHvv4gAAAABJRU5ErkJggg==)' : item}"
                                    :key="item"
                                    v-for="item in colorList"></li>
                            </ul>
                        ...
                        <div class="style-item">
                            <label>边框尺寸</label>
                            <el-input-number :step="1"
                                @change="updateItem({key:'style', val:{'border-width': $event + 'px'}})"
                                :value="parseInt(curItem.style['border-width'] || 0)"
                                :min="0"
                        ...
                        <div class="style-item">
                            <label>边框弧度</label>
                            <el-input-number :step="1"
                                @change="updateItem({key:'style', val:{'border-radius': $event + '%'}})"
                                :value="parseInt(curItem.style['border-radius'] || 0)"
                                :min="0"
        ...
        <el-scrollbar v-if="navIndex == 1"
            style="height:calc(100vh - 60px - 60px - 50px);"
            class="page-component__nav">
            <v-ani></v-ani>
        </el-scrollbar>
        <el-scrollbar v-if="navIndex == 2"
            style="height:calc(100vh - 60px - 60px - 50px);"
            class="page-component__nav">
            <v-event></v-event>
        </el-scrollbar>
